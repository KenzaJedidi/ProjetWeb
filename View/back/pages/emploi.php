...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (isset($_POST['action'])) {
      $action = $_POST['action'];
      if (isset($_GET['action']) && $_GET['action'] === 'getSalaryStats') {
        header('Content-Type: application/json');
        echo json_encode($controller->getSalaryStats());
        exit;
    }
...
<body class="g-sidenav-show bg-gray-100">
  ...
  <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg">
      ...
      <div class="container-fluid py-1 px-3">
        ...
        <div class="collapse navbar-collapse mt-sm-0 mt-2 me-md-0 me-sm-4" id="navbar">
          ...
          <ul class="navbar-nav d-flex align-items-center justify-content-end">
            ...
            <li class="nav-item d-flex align-items-center">
              <a href="statistiques_salaires.php" class="btn btn-custom btn-sm mb-0">
                <i class="material-symbols-rounded">bar_chart</i> Statistiques
              </a>
            </li>
    ...
    <div class="modal fade" id="statsModal" tabindex="-1" aria-labelledby="statsModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="statsModalLabel">Statistiques des Salaires</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
  ...
  <script>
    ...
    function loadNotifications() {
      fetch('../../../Controller/CandidatureController.php?action=getRecentCandidatures')
    ...
    function viewCandidature(candidatureId, nomComplet) {
      fetch(`../../../Controller/CandidatureController.php?action=getCandidature&id=${candidatureId}`)
        .then(response => response.json())
        .then(data => {
    ...
    document.addEventListener('click', function(event) {
      ...
      if (candidatesButton) {
        ...
        fetch(`../../../Controller/CandidatureController.php?action=getCandidaturesByOffre&offre_id=${offreId}`)
          .then(response => response.json())
          .then(data => {
            if (data.success && data.candidatures.length > 0) {
              data.candidatures.forEach(candidature => {
              ...
              document.querySelectorAll('.accept-candidature, .reject-candidature').forEach(button => {
                button.addEventListener('click', function() {
                  ...
                  fetch('../../../Controller/CandidatureController.php', {
                    method: 'POST',
                    headers: {
    ...
    // Fonction pour formater la localisation (première lettre en majuscule, reste en minuscule)
    function formatLocalisation(input) {
      if (input.value.length === 1) {
        input.value = input.value.charAt(0).toUpperCase() + input.value.slice(1).toLowerCase();
      } else if (input.value.length > 1) {
        const firstChar = input.value.charAt(0);
        const rest = input.value.slice(1);
        // Si la première lettre n'est pas en majuscule, on la corrige
        if (firstChar !== firstChar.toUpperCase()) {
          input.value = firstChar.toUpperCase() + rest.toLowerCase();
        }
      }
      validateInput(input); // Valider après formatage
    }

    // Modifiez l'input de localisation pour utiliser cette fonction
    document.getElementById('localisation').addEventListener('input', function(e) {
      formatLocalisation(this);
    });
    ...
    function validateInput(input) {
      ...
      // Cas spécial pour la localisation
      if (input.id === 'localisation') {
        const pattern = /^[A-Z][a-zÀ-ÿ\s\-,]+$/;
        isValid = pattern.test(input.value) || input.value === '';
      } else {
        const pattern = new RegExp(input.pattern);
